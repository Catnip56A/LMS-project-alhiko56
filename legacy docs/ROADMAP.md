...
**`New structure, logins, courses, many to many system of users to courses`** *(Dec 14)* — The real starting point: the full Flask application skeleton was put in place. This commit introduced `app.py`, `lms/__init__.py`, blueprints (`routes/`, `admin/`, `auth.py`, `api.py`), `models/__init__.py`, `create_user.py`, `init_db.py`, `requirements.txt`, and the first static HTML pages (`index.html`, `login.html`). Users, courses, and a many-to-many enrollment relationship were all defined here.
...
**`google drive update + google account binding` / `URI fix` / `drive_links`** *(Jan 10)* — Google accounts can now be bound to existing LMS user accounts. OAuth redirect URI fixed. Drive file links stored and surfaced in the UI.
...
**`renewal of lms features`** *(Feb 10)* — A broad features-page refresh on the home/about section: updated copy, layout improvements, new visual components.

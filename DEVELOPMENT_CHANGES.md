...
This document explains the decisions, tools, and fixes made to bring Yonca to a production-grade setup. It is written for the original developer of this project to explain what changed, why it changed, and what to keep in mind going forward.
...
```
DATABASE_URL=postgresql://yonca_user:...@localhost:5432/yonca_db
...
The codebase had several places with paths like `/home/magsud/work/Yonca/` and a hardcoded database password `ALHIKO3325!56Catnip?!` in `deploy.sh` and `reset_db.py`. This is a serious security problem — anyone who reads the source code knows your production password.
...
**Problem:** `beta.yonca-sdc.com` is hardcoded — staging, production, and any future domain would all use the wrong redirect URI. OAuth would silently fail.
...
**What was there:**
```yaml
test: ["CMD-SHELL", "pg_isready -U yonca_user"]
```

**Problem:** `pg_isready` without `-d` connects to the database named after the user (`yonca_user`). If that database doesn't exist, Postgres logs `FATAL: database "yonca_user" does not exist` — a scary error that is actually harmless (the healthcheck still passes) but extremely misleading.

**Fix:**
```yaml
test: ["CMD-SHELL", "pg_isready -U yonca_user -d yonca_db"]
```
